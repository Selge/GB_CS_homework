...
void TaskSelector()
{
    ...
    Console.WriteLine("1.Task_1 ('Задача 41').");
    Console.WriteLine("2.Task_2 ('Задача 43').");
    Console.Write("Please, type in a target Task number: ");
    int task = Convert.ToInt32(Console.ReadLine());

    if (task == 1) Aufgabe41();
    else if (task == 2) Aufgabe43();
    else
    {
...
// 1, -7, 567, 89, 223-> 3
    ...
    // string [] Start = App.ArrayFillerSelector();
    int [] Eingang = Array.ConvertAll(App.ArrayFillerSelector(), s => int.Parse(s));

    Console.WriteLine($"Your array is ({string.Join(",", Eingang)}).");
    Console.WriteLine($"Your array consists of {Eingang.Count()} numbers.");
    App.Zahlen41(Eingang);
}
...
// b1 = 2, k1 = 5, b2 = 4, k2 = 9 -> (-0,5; -0,5)
    ...
    (double b1, double k1) SatzKoordinatenA = App.KoordinatenA43();
    (double b2, double k2) SatzKoordinatenB = App.KoordinatenB43();
...
// Не вполне уверен, что использую классы правильно, но если заработает, то наверное будет удобно...
class App
{
